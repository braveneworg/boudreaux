...
| Origin request policy          | **Custom** — must forward query strings: `response-content-disposition`, `response-content-type` to the S3 origin (e.g. use the AWS-managed `AllViewer` policy, or create a custom policy that explicitly allows those query strings) |

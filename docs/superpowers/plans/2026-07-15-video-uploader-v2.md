...
```tsx
it('adds a featured artist from search results', async () => {
  /* select 'Real Artist' → onChange(['Real Artist']) */
});
it('adds a free-text featured artist on Enter', async () => {
  /* type 'Guest{Enter}' → onChange(['Guest']) */
});
it('removes a featured pill', async () => {
  /* value ['Guest'] → click Remove Guest → onChange([]) */
});
...
it('fetches and returns the parsed result on refetch', async () => {
  (fetchAndParse as Mock).mockResolvedValue({
...
it('Publish stamps today when the date is empty', () => {
  expect(shapePublish({ ...base, publishedAt: '' }, 'publish', true).publishedAt).toBe(
    formatDateForForm(new Date())
  );
});
it('Save persists the date on an already-published video', () => {
  expect(shapePublish({ ...base, publishedAt: '2026-09-01' }, 'save', false).publishedAt).toBe(
    '2026-09-01'
  );
});
```

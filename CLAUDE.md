...
- After `EnterWorktree`, every Read/Edit must target the worktree path — reusing an absolute path captured before the switch silently edits the main checkout, which tests run from the worktree never see (the worktree branch stays empty while the main checkout goes dirty). Symptom: `wc -l`/`--list` disagree with the Read/Edit view.

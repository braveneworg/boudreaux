...
- 001-release-search-player: Added TypeScript 5.9.3 (strict mode enabled) + `cmdk` (Command/combobox), `embla-carousel-react` (Carousel), `video.js` (audio playback via MediaPlayer), `lucide-react` (icons), `next/image` (optimized images), `@tanstack/react-query` (client-side data fetching)

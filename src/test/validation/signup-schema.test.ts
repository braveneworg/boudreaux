...
import signupSchema, { type FormSchemaType } from '../../app/lib/validation/signup-schema'
...
describe('signup-schema', () => {
  ...
  describe('termsAndConditions validation', () => {
    ...
    it('should reject false value', () => {
      ...
      expect(result.success).toBe(false)
      if (!result.success) {
        const termsErrors = result.error.issues.filter(issue => issue.path[0] === 'termsAndConditions')

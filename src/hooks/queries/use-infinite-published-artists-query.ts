...
interface FetchPublishedArtistsParams {
  ...
  /** Which part of the roster to list: current artists, alumni, or both. */
 ...
 *
 * @param params - Sort/roster/search/pagination inputs plus the abort signal.
 ...
 * rows for infinite scroll. `sort`, `search`, and `roster` are applied
 * server-side and are part of the query key, so changing any of them resets
 * pagination; `keepPreviousData` keeps the current rows visible during the
 ...
 * @param roster - Current artists, alumni, or both (defaults to current).
...
export const useInfinitePublishedArtistsQuery = (
  ...
  useInfiniteQuery({
    queryKey: queryKeys.artists.publishedInfinite(sort, search, roster),

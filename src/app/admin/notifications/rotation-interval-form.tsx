...
export const RotationIntervalForm = ({ currentInterval }: RotationIntervalFormProps) => {
  ...
  const { mutateAsync: updateRotationInterval, isPending: isUpdatingRotation } =
    useUpdateRotationIntervalMutation();

  const handleSave = async () => {
    setMessage(null);
    try {
      const result = await updateRotationInterval({ interval });
      if (result.success) {
  ...
  return (
    <div className="rounded-lg border p-4">
      ...
      <div className="flex items-end gap-3">
        ...
        <Button type="button" size="sm" disabled={isUpdatingRotation} onClick={handleSave}>
          {isUpdatingRotation ? (

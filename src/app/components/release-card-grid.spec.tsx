...
describe('ReleaseCardGrid', () => {
  const mockReleases = [
    ...
    },
    {
      ...
      artistName: 'Artist C',
      coverArt: null,
      releasedOn: new Date(2022, 11, 25),
  ...
  it('should forward the release date to each ReleaseCard', () => {
    render(<ReleaseCardGrid releases={mockReleases} />);

    const card1 = screen.getByTestId('release-card-release-1');
    expect(card1).toHaveAttribute('data-released-on', new Date(2024, 0, 2).toISOString());
  });

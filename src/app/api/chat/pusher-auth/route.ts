 ...
 * and not gated, then return a signed presence payload. Rate-limited per
 * IP — legitimate clients re-authorize only on (re)connect, so the polling
 * tier is far above normal use.
 */
export const POST = withRateLimit(
  pollingLimiter,
  POLLING_LIMIT
)(
  withAuth(async (request: NextRequest, _ctx, session): Promise<NextResponse> => {

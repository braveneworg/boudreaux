...
describe('GET /api/venues/[venueId]', () => {
  ...
  it('should return a venue by ID', async () => {
    ...
    const request = new NextRequest('http://localhost:3000/api/venues/venue-123');
    const response = await GET(request, createParams('venue-123'));
    const data = await response.json();

    expect(response.status).toBe(200);
    expect(data.venue.id).toBe('venue-123');
  ...
  it('should return 404 when venue not found', async () => {
    ...
    const request = new NextRequest('http://localhost:3000/api/venues/nonexistent');

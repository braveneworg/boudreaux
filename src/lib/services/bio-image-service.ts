...
/**
 * Re-hosts AI-discovered bio images into our own S3 so they are CDN-served and
 * carry the generated `_w{width}` variants — no external hotlinks.
 */
export class BioImageService {
  ...
  static async rehostWithVariants(
    ...
    if (arrayBuffer.byteLength > MAX_BYTES) {
      throw new Error('Source image exceeds the 50MB limit');
    }

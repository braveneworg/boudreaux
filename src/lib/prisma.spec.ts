...
// Mock PrismaClient to avoid real database connections. setupTests.ts globally
...
describe('prisma', () => {
  ...
  it('should be a singleton across imports', async () => {
    const { prisma: prisma1 } = await vi.importActual<typeof PrismaModule>('@/lib/prisma');
    const { prisma: prisma2 } = await vi.importActual<typeof PrismaModule>('@/lib/prisma');
  ...
  it('should maintain consistent reference', async () => {
    const { prisma: ref1 } = await vi.importActual<typeof PrismaModule>('@/lib/prisma');
    const { prisma: ref2 } = await vi.importActual<typeof PrismaModule>('@/lib/prisma');

...
- Run seed, `db push`, migrations, and studio only with `DATABASE_URL` scoped
  ...
  `prisma.config.ts` loads `.env` for anything the shell leaves unset, so the
  repo's env files must never hold the production value. Read the isolation
  rules in [`e2e/AGENTS.md`](../e2e/AGENTS.md) first; they are a hard
  constraint.

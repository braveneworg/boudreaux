 ...
 */
const getDerivedEncryptionKey = async (secret: string, salt: string): Promise<ArrayBuffer> =>
  hkdfAsync(
    'sha256',
    secret,
    salt,
 ...
 */
const createAuthCookie = async (user: TestUser, secret: string) => {
 ...
 */
const createStorageState = async (user: TestUser, secret: string) => {
